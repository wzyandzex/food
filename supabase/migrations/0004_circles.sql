...
-- ============ 成员关联表（独立表优于 uuid[]：单行增删 + 角色字段 + exists 子查询 RLS）============
...
-- 写操作全部经服务端 service_role 完成并做应用层校验（人数上限、角色权限）

-- ============ 邀请链接 token（share_tokens 同款模式，7 天有效）============
...
-- token 仅经服务端校验使用，不开放直查

-- ============ 点单会话挂接圈子（可空；解散置空保留历史）============
